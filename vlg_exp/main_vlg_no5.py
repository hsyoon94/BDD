...
def train(epoch, model, train_data_dir, train_data_name_list, optimizer, criterion, device):
    ...
    rnd_index = np.random.choice(len(train_data_name_list)-1, int(len(train_data_name_list)/100), replace=False)
    ...
    for i in range(rnd_index.shape[0]):
        ...
        # input_other_car = input_tv  # point global path
        input_my_car = input_ev
        input_static = input_gp
        ...
        input = np.vstack([input_static, input_my_car, input_static])
...
if os.path.exists('/mnt/sda2/BDD/log/5/model_' + now_date + '_' + now_time + '/') is False:
    os.mkdir('/mnt/sda2/BDD/log/5/model_' + now_date + '_' + now_time + '/')

is_cuda = torch.cuda.is_available()
device = torch.device('cuda' if is_cuda else 'cpu')

expert_demo_train_dir = '/mnt/sda2/BDD/data/5'
...
for i in range(epoch):
    ...
    myfile = open('/mnt/sda2/BDD/log/5/model_' + now_date + '_' + now_time + '/loss.txt', 'a')
    ...
    if i % 20 == 0:
        ...
        torch.save(model.state_dict(), '/mnt/sda2/BDD/log/5/model_' + now_date + '_' + now_time + '/epoch' + str(i) + '_' + str(int(loss_train_tmp)) + '.pt')
